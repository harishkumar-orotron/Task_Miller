...
import { parseCsvToImportBody, useImportProjectMutation } from '../../queries/import-export.queries'
import type { ImportProjectBody } from '../../types/import-export.types'
...
function validate(body: ImportProjectBody): FieldErrors {
  const e: FieldErrors = {}
  if (!body.project.title.trim()) e['proj.title'] = 'Required'
...
export default function ImportProjectModal({ orgId, onClose }: Props) {
  const [body,      setBody]      = useState<ImportProjectBody | null>(null)
  const [parseErrs, setParseErrs] = useState<string[]>([])
  const [fileName,  setFileName]  = useState('')
  const [touched,   setTouched]   = useState(false)
  const fileInputRef = useRef<HTMLInputElement>(null)

  const { mutate: importProject, isPending, error: mutationError } = useImportProjectMutation()
  ...
  function handleFile(file: File) {
    ...
    reader.onload = (e) => {
      ...
      const result = parseCsvToImportBody(text)
  ...
  function updateProject(field: string, value: string) {
    setBody((prev) => prev ? { ...prev, project: { ...prev.project, [field]: value } } : prev)
  }
  ...
  function handleSubmit() {
    if (!body || !orgId || hasErrors) return
    importProject({ orgId, body }, { onSuccess: onClose })
  }
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col max-h-[90vh]">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 flex-shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="text-base font-semibold text-gray-800">Import Full Project</h2>
        </div>
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              {body ? 'Click to replace file' : 'Only exported project CSV files are supported'}
            </p>
          </div>
        ...
        {parseErrs.length > 0 && (
          <div className="bg-red-50 border border-red-200 rounded-lg px-3 py-3 space-y-1">
            <div className="flex items-center gap-1.5 text-red-600 font-medium text-xs">
              <AlertCircle size={13} /> Invalid CSV — fix these issues and re-upload
            </div>
            {parseErrs.map((err, i) => (
          ...
          <div className="space-y-4">
            ...
            {touched && hasErrors && (
              <div className="bg-amber-50 border border-amber-200 text-amber-700 text-xs px-3 py-2 rounded-lg flex items-center gap-1.5">
                <AlertCircle size={12} />
                {Object.keys(fieldErrors).length} field{Object.keys(fieldErrors).length !== 1 ? 's' : ''} need attention — fix them below before importing
              </div>
            )}
            ...
            <div className="border border-gray-100 rounded-xl overflow-hidden">
              ...
              <div className="px-4 py-3 flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <input
                    value={body.project.title}
                    onChange={(e) => updateProject('title', e.target.value)}
                    className={`${fieldCls('proj.title')} w-full`}
                    placeholder="Project title"
                  />
                  {touched && fieldErrors['proj.title'] && (
                    <span className={labelErrCls}>{fieldErrors['proj.title']}</span>
                  )}
                </div>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="border border-gray-200 rounded px-2 py-1 text-xs outline-none focus:border-orange-400 bg-white resize-none w-full"
      ...
      <div className="flex gap-3 px-5 py-4 border-t border-gray-100 flex-shrink-0">
        ...
        >
          {isPending ? 'Importing...' : 'Import Project'}
        </button>
      </div>
